...
namespace KVectorUtil
{
    ...
    void DrawLine(HDC hdc, double x0, double y0, double x1, double y1
        , int lineWidth = 1, int penStyle = PS_SOLID, COLORREF color = RGB(0, 0, 0));
    ...
    void DrawCircle(HDC hdc, const KVector2& center, double radius, int numSegment
        , int lineWidth = 1, int penStyle = PS_SOLID, COLORREF color = RGB(0, 0, 0));
    ...
    void ScanLineSegment( HDC hdc, double x1, double y1, KRgb c1, double x2, double y2, KRgb c2
        , std::set<ScannedResult>* output );

    void FillTriangle(HDC hdc, int x1, int y1, KRgb const col1
        , int x2, int y2, KRgb const col2
        , int x3, int y3, KRgb const col3);
    void FillTriangle( HDC hdc, double x1, double y1, KRgb const col1
        , double x2, double y2, KRgb const col2
        , double x3, double y3, KRgb const col3 );

    double LengthSquared(const KVector2& a, const KVector2& b);
    double Length(const KVector2& a, const KVector2& b);
    double PointLinesegmentDistance(KVector2 p, KVector2 v0, KVector2 v1);
    double PointLineDistance(KVector2 p, KVector2 v0, KVector2 v1);
    ...
    void ScanDrawFilledPolygon(HDC hdc, std::vector<KVector2>& vertices, KRgb color);
        /// <summary>
